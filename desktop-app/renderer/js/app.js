...
const TOOL_DEFS = [
  ...
  {
    name: 'plugin_arduino-cli-toolchain_list_platforms', category: 'query',
    description: '列出已安装的 Arduino 开发板平台（arduino-cli core list），如 arduino:avr、esp32:esp32。用于确认某个平台是否需要安装',
    parameters: { type: 'object', properties: {}, required: [] },
    execute: async () => {
  ...
  {
    name: 'plugin_arduino-cli-toolchain_search_boards', category: 'query',
    description: '搜索 Arduino 开发板（arduino-cli board search），参数 query 如 "ESP32C3"、"STM32"。返回可用的板名/FQBN/平台ID，用于选板',
    parameters: { type: 'object', properties: { query: { type: 'string', description: '搜索关键词，如 ESP32、ESP32C3、STM32' } }, required: ['query'] },
    execute: async (args) => {
  ...
  {
    name: 'plugin_arduino-cli-toolchain_select_board', category: 'modify',
    description: '选择 Arduino 开发板（搜索并记住 FQBN，后续 compile 使用）。参数 query 如 "ESP32C3"。选择后无需重复选择，除非更换板子',
    parameters: { type: 'object', properties: { query: { type: 'string', description: '板子搜索关键词，如 ESP32、ESP32C3、STM32' } }, required: ['query'] },
    execute: async (args) => {
      if (!args.query) return 'Error: 缺少 query 参数';
      const r = await runArduinoCli(['board', 'search', String(args.query)]);
      if (!r.ok) return r.out;
      // 解析 FQBN：board search 输出行含 FQBN（如 esp32:esp32:esp32c3 / adafruit:samd:adafruit_qtpy_esp32c3）
      ...
      // 优先：行文本（板名/FQBN）包含查询关键词；其次：FQBN 最后一段包含关键词；兜底第一个
      ...
      if (!state.arduinoFqbn) state.arduinoFqbn = fqbn;
      else state.arduinoFqbn = fqbn;
      return '已选择开发板: ' + fqbn + '（后续 compile 将使用该 FQBN；如需查看板子细节可用 list_platforms 确认平台已安装）';
    }
  },
  {
    name: 'plugin_arduino-cli-toolchain_install_platform', category: 'modify',
    description: '安装 Arduino 开发板平台（arduino-cli core install），参数 platform 如 "esp32:esp32"、"stm32:stm32"。编译某板子报"platform not installed"时先安装',
    parameters: { type: 'object', properties: { platform: { type: 'string', description: '平台ID，如 esp32:esp32（可先用 list_platforms / board search 确认）' } }, required: ['platform'] },
    execute: async (args) => {
      ...
      return (ok ? '平台安装成功: ' + args.platform + '\n' : '平台安装返回（请查看输出）: ' + args.platform + '\n') + r.out.slice(0, 1500);
    }
  },
  {
    name: 'plugin_arduino-cli-toolchain_install_library', category: 'modify',
    description: '安装 Arduino 库（arduino-cli lib install），参数 name 如 "DHT sensor library"、"Adafruit SSD1306"。编译报头文件缺失时先安装对应库',
    parameters: { type: 'object', properties: { name: { type: 'string', description: '库名称，如 DHT sensor library、Adafruit SSD1306' } }, required: ['name'] },
    execute: async (args) => {
      if (!args.name) return 'Error: 缺少 name 参数';
      const r = await runArduinoCli(['lib', 'install', String(args.name)], 300000);
      if (!r.ok) return r.out;
      return (r.code === 0 ? '库安装成功: ' + args.name + '\n' : '库安装返回（请查看输出）: ' + args.name + '\n') + r.out.slice(0, 1200);
    }
  },
  {
    name: 'plugin_arduino-cli-toolchain_list_installed_libraries', category: 'query',
    description: '列出已安装的 Arduino 库（arduino-cli lib list），用于确认依赖库是否已就绪',
    parameters: { type: 'object', properties: {}, required: [] },
    execute: async () => {
  ...
  {
    name: 'plugin_arduino-cli-toolchain_search_libraries', category: 'query',
    description: '搜索 Arduino 库（arduino-cli lib search），参数 query 如 "DHT"、"OLED"。返回候选库名/版本/描述',
    parameters: { type: 'object', properties: { query: { type: 'string', description: '库搜索关键词，如 DHT、SSD1306' } }, required: ['query'] },
    execute: async (args) => {
      if (!args.query) return 'Error: 缺少 query 参数';
  ...
  {
    name: 'plugin_arduino-cli-toolchain_compile', category: 'execute',
    description: '编译当前 Arduino 项目（真实 arduino-cli）。优先使用 select_board 选择的 FQBN，未选择时自动识别 esp32/esp32c3。返回编译结果与资源占用；失败时按输出修复代码后重试',
    parameters: { type: 'object', properties: {}, required: [] },
    execute: async () => {
      const cwd = state.projectPath;
      ...
      // 定位主 .ino（根目录 → src/ → 内存 state.files 兜底）
      let inoFile = '';
      try {
        const safeList = async (dir) => {
          try {
      ...
      // 临时 sketch 目录编译（主 .ino 必须与目录同名 + 避免项目内残片干扰）
      let tmpSketchDir = '', tmpInoName = '', compileArg = cwd;
      try {
        const tmpDirName = 'labcode_build_' + Date.now();
        tmpSketchDir = cwd + '\\' + tmpDirName;
        tmpInoName = tmpDirName + '.ino';
        ...
        const copyRes = await window.LabCode.fs.copyFile(cwd + '\\' + inoFile, tmpSketchDir + '\\' + tmpInoName);
        const existsCheck = await window.LabCode.fs.exists(tmpSketchDir + '\\' + tmpInoName);
        if (copyRes && copyRes.success && existsCheck) compileArg = tmpSketchDir;
        else { tmpSketchDir = ''; }
      } catch (e) { tmpSketchDir = ''; }
      const cmd = cli.cmd + ' compile --fqbn ' + fqbn + ' "' + compileArg + '"';
      let output = '';
      try {
        const result = await window.LabCode.terminal.execute(cmd, cwd, 300000);
      ...
      const errLine = output.split('\n').find(l => /:\d+:\d+:\s*error:|:\d+:\s*error:|error:\s+/i.test(l));
      // ===== read_diagnostics 数据源：缓存最近一次编译诊断（对齐 TrieCode read_diagnostics）=====
...
13. **自主迭代与问题自愈（像豆包一样主动解决）**：遇到编译/运行/配置错误时不要停在报错上，先自行分析根因并尝试修复（修改代码→重跑验证，可多轮迭代直到通过）。确认缺少依赖或工具链时：① 先调用 list_plugins 查看已安装/可安装的插件能力；② 如果缺的是 LabCode 插件能解决的（AI 模型、编译工具链、串口等），先用 ask_user 询问用户「是否允许安装 X（说明用途、大小、耗时）」，用户同意后调用 install_plugin 安装，装完继续执行任务；③ 如果缺的是系统级软件（python、gcc、库等）且插件市场没有，评估是否可用 terminal 安装（需先向用户说明并确认）；④ 用户拒绝时说明影响并给出替代方案。不要假装问题已解决，也不要跳过验证直接宣称完成——每轮修复后必须重新编译/运行验证，拿到真实结果再收尾。`;
  ...
  _streamChat(chatMessages) {
    return new Promise((resolve) => {
      ...
      // ===== 2026-09-13 修复：活性看门狗——60s 无任何流事件 → 视为挂起，回退非流式请求 =====
      // 本地 9B 模型/主进程转发偶发断流，300s 超时太久用户感知"卡死"；先降级再重试
      let lastEventAt = Date.now();
      const watchdog = setInterval(() => {
        if (Date.now() - lastEventAt > 60000) {
          if (!settled) {
            settled = true; cleanup();
            addOutputLog('流式响应无数据（60s 无事件），回退非流式请求', 'warn');
            resolve({ ok: false, error: '流式响应无数据（60s 无事件），回退非流式请求' });
          }
        }
      }, 15000);
